<!doctype html>
<html lang="{{ $currentLocale ?? app()->getLocale() }}" dir="{{ $isRtl ?? true ? 'rtl' : 'ltr' }}">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <title>@yield('title', $isRtl ? 'حسابي' : 'My Account') | {{ config('app.name') }}</title>

    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link
        href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;500;600;700;800&family=Inter:wght@400;500;600;700&display=swap"
        rel="stylesheet">
    <style>
        body {
            font-family: "Cairo", sans-serif;
            font-optical-sizing: auto;
            font-weight: 700;
            font-style: normal;
            font-variation-settings: "slnt" -2;
        }
    </style>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
    <style>
        :root {
            --c-sidebar-w: 260px;
            --c-bg: #f5f7fb;
            --c-border: #e6e9f0;
            --c-text: #1e2333;
            --c-muted: #6b7280;
            --c-primary: #0d6efd;
            --c-success: #16a34a;
            --c-danger: #dc2626;
            --c-warning: #f59e0b;
        }

        * {
            box-sizing: border-box;
        }

        body {
            margin: 0;
            font-family: {{ $isRtl ?? true ? "'Cairo'" : "'Inter'" }}, system-ui, sans-serif;
            background: var(--c-bg);
            color: var(--c-text);
        }

        a {
            color: inherit;
        }

        .client-layout {
            display: flex;
            min-height: 100vh;
        }

        .client-sidebar {
            width: var(--c-sidebar-w);
            background: #0f172a;
            color: #cbd5e1;
            padding: 1.25rem 0;
            position: fixed;
            top: 0;
            bottom: 0;
            {{ $isRtl ?? true ? 'right' : 'left' }}: 0;
            overflow-y: auto;
            z-index: 100;
        }

        .client-sidebar .brand {
            padding: 0 1.25rem 1.25rem;
            border-bottom: 1px solid #1e293b;
            margin-bottom: 1rem;
            display: flex;
            align-items: center;
            gap: .6rem;
        }

        .client-sidebar .brand-logo {
            width: 38px;
            height: 38px;
            background: linear-gradient(135deg, #0d6efd, #6366f1);
            border-radius: 10px;
            display: grid;
            place-items: center;
            color: white;
            font-weight: 800;
            font-size: 1.1rem;
        }

        .client-sidebar .brand-name {
            color: white;
            font-weight: 700;
            font-size: 1rem;
        }

        .client-sidebar .user-box {
            padding: 1rem 1.25rem;
            margin: 0 .5rem 1rem;
            background: #1e293b;
            border-radius: 10px;
            text-align: center;
        }

        .client-sidebar .user-avatar {
            width: 56px;
            height: 56px;
            border-radius: 50%;
            background: linear-gradient(135deg, #0d6efd, #6366f1);
            display: grid;
            place-items: center;
            color: white;
            font-weight: 800;
            font-size: 1.4rem;
            margin: 0 auto .6rem;
        }

        .client-sidebar .user-name {
            color: white;
            font-weight: 700;
            font-size: .95rem;
        }

        .client-sidebar .user-email {
            color: #94a3b8;
            font-size: .8rem;
            margin-top: .15rem;
            word-break: break-all;
        }

        .client-sidebar .nav-section {
            padding: 0 .5rem;
            margin-bottom: .5rem;
        }

        .client-sidebar .nav-title {
            font-size: .7rem;
            text-transform: uppercase;
            color: #64748b;
            padding: .5rem 1rem;
            letter-spacing: .05em;
        }

        .client-sidebar .nav-link {
            display: flex;
            align-items: center;
            gap: .7rem;
            padding: .65rem 1rem;
            border-radius: 8px;
            color: #cbd5e1;
            text-decoration: none;
            font-size: .92rem;
            font-weight: 500;
            transition: all .15s;
        }

        .client-sidebar .nav-link:hover {
            background: #1e293b;
            color: white;
        }

        .client-sidebar .nav-link.active {
            background: linear-gradient(135deg, #0d6efd, #6366f1);
            color: white;
        }

        .client-sidebar .nav-link i {
            width: 20px;
            text-align: center;
        }

        .client-main {
            flex: 1;
            margin-{{ $isRtl ?? true ? 'right' : 'left' }}: var(--c-sidebar-w);
            display: flex;
            flex-direction: column;
        }

        .client-topbar {
            background: white;
            border-bottom: 1px solid var(--c-border);
            padding: .85rem 1.5rem;
            display: flex;
            align-items: center;
            justify-content: space-between;
            position: sticky;
            top: 0;
            z-index: 50;
        }

        .client-topbar h1 {
            font-size: 1.15rem;
            margin: 0;
            font-weight: 700;
        }

        .client-topbar .actions {
            display: flex;
            gap: .75rem;
            align-items: center;
        }

        .client-topbar .user-chip {
            display: flex;
            align-items: center;
            gap: .5rem;
            padding: .4rem .8rem;
            background: var(--c-bg);
            border-radius: 999px;
            font-size: .88rem;
            font-weight: 600;
        }

        .client-topbar .user-chip i {
            color: var(--c-primary);
        }

        .client-content {
            padding: 1.5rem;
            flex: 1;
        }

        .client-card {
            background: white;
            border: 1px solid var(--c-border);
            border-radius: 12px;
            padding: 1.25rem;
        }

        .client-card+.client-card {
            margin-top: 1rem;
        }

        .client-card .card-head {
            display: flex;
            justify-content: space-between;
            align-items: center;
            margin-bottom: 1rem;
            padding-bottom: .85rem;
            border-bottom: 1px solid var(--c-border);
        }

        .client-card .card-head h2 {
            font-size: 1.05rem;
            margin: 0;
            font-weight: 700;
        }

        .stat-grid {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));
            gap: 1rem;
            margin-bottom: 1.5rem;
        }

        .stat-card {
            background: white;
            border: 1px solid var(--c-border);
            border-radius: 12px;
            padding: 1.25rem;
            display: flex;
            align-items: center;
            gap: 1rem;
        }

        .stat-card .stat-icon {
            width: 50px;
            height: 50px;
            border-radius: 10px;
            display: grid;
            place-items: center;
            font-size: 1.25rem;
        }

        .stat-card .stat-value {
            font-size: 1.7rem;
            font-weight: 800;
            margin: 0;
            line-height: 1;
        }

        .stat-card .stat-label {
            color: var(--c-muted);
            font-size: .85rem;
            margin-top: .35rem;
        }

        .stat-icon.bg-primary {
            background: #dbeafe;
            color: #1d4ed8;
        }

        .stat-icon.bg-success {
            background: #dcfce7;
            color: #166534;
        }

        .stat-icon.bg-warning {
            background: #fef3c7;
            color: #a16207;
        }

        .stat-icon.bg-danger {
            background: #fee2e2;
            color: #991b1b;
        }

        .stat-icon.bg-info {
            background: #e0e7ff;
            color: #4338ca;
        }

        .welcome-banner {
            background: linear-gradient(135deg, #0d6efd, #6366f1);
            color: white;
            border-radius: 12px;
            padding: 1.75rem 1.75rem;
            margin-bottom: 1.5rem;
            display: flex;
            justify-content: space-between;
            align-items: center;
            gap: 1rem;
            flex-wrap: wrap;
        }

        .welcome-banner h2 {
            margin: 0 0 .35rem;
            font-size: 1.5rem;
            font-weight: 800;
        }

        .welcome-banner p {
            margin: 0;
            opacity: .9;
        }

        .client-table {
            width: 100%;
            border-collapse: collapse;
        }

        .client-table th,
        .client-table td {
            padding: .8rem .75rem;
            text-align: {{ $isRtl ?? true ? 'right' : 'left' }};
            border-bottom: 1px solid var(--c-border);
            font-size: .9rem;
        }

        .client-table th {
            background: #f9fafb;
            font-weight: 600;
            color: var(--c-muted);
            font-size: .8rem;
            text-transform: uppercase;
            letter-spacing: .04em;
        }

        .client-table tr:hover td {
            background: #fafbff;
        }

        .btn-c {
            display: inline-flex;
            align-items: center;
            gap: .4rem;
            padding: .5rem .9rem;
            border-radius: 8px;
            font-size: .88rem;
            font-weight: 600;
            border: 1px solid transparent;
            cursor: pointer;
            text-decoration: none;
            transition: all .15s;
        }

        .btn-c-primary {
            background: var(--c-primary);
            color: white;
        }

        .btn-c-primary:hover {
            background: #0b5ed7;
            color: white;
        }

        .btn-c-outline {
            background: white;
            color: var(--c-text);
            border-color: var(--c-border);
        }

        .btn-c-outline:hover {
            background: var(--c-bg);
        }

        .btn-c-sm {
            padding: .3rem .6rem;
            font-size: .8rem;
        }

        .badge-pill {
            display: inline-block;
            padding: .25rem .7rem;
            border-radius: 999px;
            font-size: .75rem;
            font-weight: 700;
        }

        .form-row {
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 1rem;
            margin-bottom: 1rem;
        }

        .form-row.full {
            grid-template-columns: 1fr;
        }

        .form-field label {
            display: block;
            font-size: .85rem;
            font-weight: 600;
            margin-bottom: .4rem;
            color: var(--c-text);
        }

        .form-field input,
        .form-field select,
        .form-field textarea {
            width: 100%;
            padding: .6rem .8rem;
            border: 1px solid var(--c-border);
            border-radius: 8px;
            font-size: .92rem;
            font-family: inherit;
        }

        .form-field .err {
            color: var(--c-danger);
            font-size: .8rem;
            margin-top: .3rem;
        }

        .form-field input:disabled {
            background: #f3f4f6;
            color: #6b7280;
        }

        .alert {
            padding: .8rem 1rem;
            border-radius: 8px;
            margin-bottom: 1rem;
            font-size: .9rem;
        }

        .alert-success {
            background: #dcfce7;
            color: #166534;
            border: 1px solid #86efac;
        }

        .alert-danger {
            background: #fee2e2;
            color: #991b1b;
            border: 1px solid #fca5a5;
        }

        .info-grid {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));
            gap: 1rem;
        }

        .info-item {
            padding: .75rem 0;
            border-bottom: 1px solid #f0f0f0;
        }

        .info-item .k {
            color: var(--c-muted);
            font-size: .8rem;
            margin-bottom: .2rem;
        }

        .info-item .v {
            font-weight: 700;
        }

        .empty {
            text-align: center;
            padding: 3rem 1.5rem;
        }

        .empty i {
            font-size: 3rem;
            color: #cbd5e1;
        }

        .empty p {
            color: var(--c-muted);
            margin: 1rem 0;
        }

        @media (max-width: 900px) {
            .client-sidebar {
                transform: translateX({{ $isRtl ?? true ? '100%' : '-100%' }});
                transition: transform .25s;
            }

            .client-sidebar.open {
                transform: translateX(0);
            }

            .client-main {
                margin-{{ $isRtl ?? true ? 'right' : 'left' }}: 0;
            }

            .form-row {
                grid-template-columns: 1fr;
            }
        }
    </style>

    @stack('styles')
</head>

<body>

    @php
        $user = auth()->user();
        $initial = mb_strtoupper(mb_substr($user->name ?? '?', 0, 1));
        $nav = [
            [
                'section' => $isRtl ? 'حسابي' : 'My Account',
                'items' => [
                    ['route' => 'client.dashboard', 'icon' => 'fa-gauge', 'label' => $isRtl ? 'الرئيسية' : 'Dashboard'],
                    [
                        'route' => 'client.profile',
                        'icon' => 'fa-user',
                        'label' => $isRtl ? 'بياناتي الشخصية' : 'My Profile',
                    ],
                ],
            ],
            [
                'section' => $isRtl ? 'الخدمات' : 'Services',
                'items' => [
                    [
                        'route' => 'client.financing-request.index',
                        'icon' => 'fa-file-signature',
                        'label' => $isRtl ? 'طلبات التمويل' : 'Financing Requests',
                    ],
                    ['route' => 'services.index', 'icon' => 'fa-briefcase', 'label' => $isRtl ? 'الخدمات' : 'Services'],
                ],
            ],
        ];
    @endphp

    <div class="client-layout">
        <aside class="client-sidebar" id="clientSidebar">
            <div class="brand">
                <div class="brand-logo">H</div>
                <div class="brand-name">{{ $siteSettings->get('site_name_' . app()->getLocale(), config('app.name')) }}
                </div>
            </div>

            <div class="user-box">
                <div class="user-avatar">{{ $initial }}</div>
                <div class="user-name">{{ $user->name }}</div>
                <div class="user-email">{{ $user->email }}</div>
            </div>

            @foreach ($nav as $section)
                <div class="nav-section">
                    <div class="nav-title">{{ $section['section'] }}</div>
                    @foreach ($section['items'] as $item)
                        @php $active = request()->routeIs($item['route']); @endphp
                        <a href="{{ \Route::has($item['route']) ? route($item['route']) : '#' }}"
                            class="nav-link {{ $active ? 'active' : '' }}">
                            <i class="fa-solid {{ $item['icon'] }}"></i>
                            <span>{{ $item['label'] }}</span>
                        </a>
                    @endforeach
                </div>
            @endforeach

            <div class="nav-section" style="margin-top: 2rem;">
                <a href="{{ route('home') }}" class="nav-link">
                    <i class="fa-solid fa-arrow-up-right-from-square"></i>
                    <span>{{ $isRtl ? 'عرض الموقع' : 'View Site' }}</span>
                </a>
                <form method="POST" action="{{ route('logout') }}" style="margin: 0;">
                    @csrf
                    <button type="submit" class="nav-link"
                        style="background: none; border: 0; width: 100%; text-align: {{ $isRtl ? 'right' : 'left' }}; cursor: pointer; font: inherit; color: inherit;">
                        <i class="fa-solid fa-right-from-bracket"></i>
                        <span>{{ $isRtl ? 'تسجيل الخروج' : 'Logout' }}</span>
                    </button>
                </form>
            </div>
        </aside>

        <div class="client-main">
            <header class="client-topbar">
                <h1>@yield('page_title', $isRtl ? 'لوحة تحكم العميل' : 'Client Dashboard')</h1>
                <div class="actions">
                    <a href="{{ route('locale.switch', app()->getLocale() === 'ar' ? 'en' : 'ar') }}"
                        class="btn-c btn-c-outline btn-c-sm">
                        <i class="fa-solid fa-language"></i> {{ app()->getLocale() === 'ar' ? 'EN' : 'AR' }}
                    </a>
                    <div class="user-chip">
                        <i class="fa-solid fa-user"></i>
                        {{ $user->name }}
                    </div>
                </div>
            </header>

            <main class="client-content">
                @if (session('success'))
                    <div class="alert alert-success">{{ session('success') }}</div>
                @endif
                @if (session('error') || $errors->any())
                    <div class="alert alert-danger">
                        {{ session('error') ?? ($isRtl ? 'يوجد أخطاء في البيانات المدخلة' : 'Please correct the errors below') }}
                    </div>
                @endif

                @yield('content')
            </main>
        </div>
    </div>

    @stack('scripts')
</body>

</html>
